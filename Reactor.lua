...
local reactor
local fluxgate
local inputfluxgatetop
local inputfluxgatebottom
...
inputfluxgatetop = f.periphSearch("flux_gate_8")
inputfluxgatebottom = f.periphSearch("flux_gate_7")
...
if inputfluxgatetop == null then
error("No valid flux gate was found")
end

if inputfluxgatebottom == null then
error("No valid flux gate was found")
end
...
if yPos == 10 and autoInputGate == 0 and xPos ~= 14 and xPos ~= 15 then
  ...
  inputfluxgatetop.setSignalLowFlow(curInputGate)
  end
  inputfluxgatebottom.setSignalLowFlow(curInputGate)
  save_config()
end
...
print("Input Gate Top: ", inputfluxgatetop.getSignalLowFlow())
print("Input Gate Bottom: ", inputfluxgatebottom.getSignalLowFlow())
...
f.draw_text_lr(mon, 2, 4, 1, "Generation", f.format_int(ri.generationRate) .. " RF/t/t", colors.white, colors.lime, colors.black)
...
f.draw_text_lr(mon, 2, 9, 1, "Input Gate Top", f.format_int(inputfluxgatetop.getSignalLowFlow()) .. " RF/t", colors.white, colors.blue, colors.black)

f.draw_text_lr(mon, 3, 7, 1, "Input Gate Bottom", f.format_int(inputfluxgatebottom.getSignalLowFlow()) .. " RF/t", colors.white, colors.blue, colors.black)
...
if ri.status == "warming_up" then
  inputfluxgatetop.setSignalLowFlow(900000)
  inputfluxgatebottom.setSignalLowFlow(900000)
...
if ri.status == "running" then
  if autoInputGate == 1 then 
    ...
    inputfluxgatetop.setSignalLowFlow(fluxval)
	inputfluxgatebottom.setSignalLowFlow(fluxval)
  else
    inputfluxgatetop.setSignalLowFlow(curInputGate)
	inputfluxgatebottom.setSignalLowFlow(curInputGate)
  end
